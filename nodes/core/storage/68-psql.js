...
module.exports = function(VARAI) {
    ...
    function psqlin(n) {
        ...
        var server = dgram.createSocket('psql4');

        server.on("error", function (err) {
            if ((err.code == "EACCES") && (node.port < 1024)) {
                node.error("psql access error, you may need root access for ports below 1024");
            } else {
                node.error("psql error : "+err.code);
            }
            server.close();
        });
        ...
        server.on('listening', function () {
            ...
            node.log('psql listener at ' + address.address + ":" + address.port);
            if (node.multicast == "true") {
                server.setBroadcast(true);
                try {
                    server.setMulticastTTL(128);
                    server.addMembership(node.group,node.iface);
                    node.log("psql multicast group "+node.group);
                } catch (e) {
                    if (e.errno == "EINVAL") {
        ...
        node.on("close", function() {
            try {
                server.close();
                node.log('psql listener stopped');
            } catch (err) {
                node.error(err);
            }
        });
    ...
    function psqlout(n) {
        ...
        var sock = dgram.createSocket('psql4');  // only use ipv4 for now
            ...
            sock.bind(node.outport, function() {    // have to bind before you can enable broadcast...
                ...
                if (node.multicast == "multi") {
                    try {
                        ...
                        node.log('psql multicast ready : '+node.outport+' -> '+node.addr+":"+node.port);
                    } catch (e) {
                        if (e.errno == "EINVAL") {
                            node.error("Bad Multicast Address");
                        } else if (e.errno == "ENODEV") {
                    ...
                    node.log('psql broadcast ready : '+node.outport+' -> '+node.addr+":"+node.port);
                }
            });
        } else if (node.outport != "") {
            sock.bind(node.outport);
            node.log('psql ready : '+node.outport+' -> '+node.addr+":"+node.port);
        } else {
            node.log('psql ready : '+node.addr+":"+node.port);
        }
        ...
        node.on("input", function(msg) {
            if (msg.payload != null) {
                ...
                if (add == "") {
                    node.warn("psql: ip address not set");
                } else if (por == 0) {
                    node.warn("psql: port not set");
                } else if (isNaN(por) || (por < 1) || (por > 65535)) {
                    node.warn("psql: port number not valid");
                } else {
                    var message;
                    if (node.base64) {
