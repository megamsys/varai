...
module.exports = function(VARAI) {
    ...
    VARAI.nodes.registerType("java in",javain);
    ...
    function javaout(n) {
        ...
        var sock = dgram.createSocket('java4');  // only use ipv4 for now
            ...
            sock.bind(node.outport, function() {    // have to bind before you can enable broadcast...
                ...
                if (node.multicast == "multi") {
                    try {
                        ...
                        node.log('java multicast ready : '+node.outport+' -> '+node.addr+":"+node.port);
                    } catch (e) {
                        if (e.errno == "EINVAL") {
                            node.error("Bad Multicast Address");
                        } else if (e.errno == "ENODEV") {
                    ...
                    node.log('java broadcast ready : '+node.outport+' -> '+node.addr+":"+node.port);
                }
            });
        } else if (node.outport != "") {
            sock.bind(node.outport);
            node.log('java ready : '+node.outport+' -> '+node.addr+":"+node.port);
        } else {
            node.log('java ready : '+node.addr+":"+node.port);
        }
        ...
        node.on("input", function(msg) {
            if (msg.payload != null) {
                ...
                if (add == "") {
                    node.warn("java: ip address not set");
                } else if (por == 0) {
                    node.warn("java: port not set");
                } else if (isNaN(por) || (por < 1) || (por > 65535)) {
                    node.warn("java: port number not valid");
                } else {
                    var message;
                    if (node.base64) {
        ...
        node.on("close", function() {
            try {
                sock.close();
                node.log('java output stopped');
            } catch (err) {
                node.error(err);
            }
        });
    }
    VARAI.nodes.registerType("java out",javaout);
}
