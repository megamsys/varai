    ...
    varaiApp.post(new RegExp("/library/flows\/(.*)"), function(req,res) {
            ...
            req.on('end', function() {
                storage.saveFlow(req.params[0],fullBody).then(function() {
                    ...
                    util.log("[varai] Error loading flow '"+req.params[0]+"' : "+err);
                    res.send(500);
                });
            });
    });
    
    varaiApp.get("/library/flows",function(req,res) {
            storage.getAllFlows().then(function(flows) {
                res.json(flows);
            });
    });
    
    varaiApp.get(new RegExp("/library/flows\/(.*)"), function(req,res) {
...
function createLibrary(type) {
    ...
    varaiApp.get(new RegExp("/library/"+type+"($|\/(.*))"),function(req,res) {
            ...
            storage.getLibraryEntry(type,path).then(function(result) {
                ...
                if (err) {
                    util.log("[varai] Error loading library entry '"+path+"' : "+err);
                }
                res.send(404);
            });
    });
    
    varaiApp.post(new RegExp("/library/"+type+"\/(.*)"),function(req,res) {
            ...
            req.on('end', function() {
                    storage.saveLibraryEntry(type,path,req.query,fullBody).then(function() {
                        ...
                        util.log("[varai] Error saving library entry '"+path+"' : "+err);
                        res.send(500);
                    });
            });
    });
