...
VARAI.sidebar.config = function() {
    ...
    $("#btn-config-nodes").click(function(){
        if (!VARAI.sidebar.containsTab("config")) {
            VARAI.sidebar.addTab("config",content,true);
        }
        refresh();
        VARAI.sidebar.show("config");
    });
    
    function refresh() {
        list.empty();
        VARAI.nodes.eachConfig(function(node) {
            ...
            entry.on('dblclick',function(e) {
                VARAI.editor.editConfig("", node.type, node.id);
            });
