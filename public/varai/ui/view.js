...
VARAI.view = function() {
    ...
    var outer = d3.select("#chart")
     ...
     var vis = outer
        ...
        .on("touchend", function() {
            ...
            touchStartTime = null;
            if  (VARAI.touch.radialMenu.active()) {
                return;
            }
            if (lasso) {
    ...
    var workspace_tabs = VARAI.tabs.create({
        id: "workspace-tabs",
    ...
    function deleteWorkspace(id) {
        ...
        var ws = VARAI.nodes.workspace(id);
    ...
    function canvasMouseMove() {
        ...
        if (mouse_mode != VARAI.state.IMPORT_DRAGGING && !mousedown_node && selected_link == null) return;
        ...
        if (mouse_mode == VARAI.state.JOINING) {
            ...
            d3.event.preventDefault();
        } else if (mouse_mode == VARAI.state.MOVING) {
            var m = mouse_position;
            var d = (mouse_offset[0]-m[0])*(mouse_offset[0]-m[0]) + (mouse_offset[1]-m[1])*(mouse_offset[1]-m[1]);
            if (d > 2) {
                mouse_mode = VARAI.state.MOVING_ACTIVE;
                clickElapsed = 0;
            }
        } else if (mouse_mode == VARAI.state.MOVING_ACTIVE || mouse_mode == VARAI.state.IMPORT_DRAGGING) {
            var mousePos = mouse_position;
            var minX = 0;
            var minY = 0;
    ...
    function canvasMouseUp() {
        if (mousedown_node && mouse_mode == VARAI.state.JOINING) {
        ...
        if (lasso) {
            ...
            lasso.remove();
            lasso = null;
        } else if (mouse_mode == VARAI.state.DEFAULT && mousedown_link == null) {
            clearSelection();
            updateSelection();
        }
        if (mouse_mode == VARAI.state.MOVING_ACTIVE) {
            if (moving_set.length > 0) {
                var ns = [];
                for (var i in moving_set) {
                    ns.push({n:moving_set[i].n,ox:moving_set[i].ox,oy:moving_set[i].oy});
                }
                VARAI.history.push({t:'move',nodes:ns,dirty:dirty});
            }
        }
        if (mouse_mode == VARAI.state.MOVING || mouse_mode == VARAI.state.MOVING_ACTIVE) {
            for (var i=0;i<moving_set.length;i++) {
                delete moving_set[i].ox;
                delete moving_set[i].oy;
            }
        }
        if (mouse_mode == VARAI.state.IMPORT_DRAGGING) {
    ...
    $("#chart").droppable({
            ...
            drop: function( event, ui ) {
                ...
                nn._def = VARAI.nodes.getType(nn.type);
                ...
                VARAI.history.push({t:'add',nodes:[nn.id],dirty:dirty});
                VARAI.nodes.add(nn);
                VARAI.editor.validateNode(nn);
                setDirty(true);
    ...
    function updateSelection() {
        ...
        if (moving_set.length == 0 && selected_link == null) {
            VARAI.keyboard.remove(/* backspace */ 8);
            VARAI.keyboard.remove(/* delete */ 46);
            VARAI.keyboard.remove(/* c */ 67);
            VARAI.keyboard.remove(/* x */ 88);
        } else {
            VARAI.keyboard.add(/* backspace */ 8,function(){deleteSelection();d3.event.preventDefault();});
            VARAI.keyboard.add(/* delete */ 46,function(){deleteSelection();d3.event.preventDefault();});
            VARAI.keyboard.add(/* c */ 67,{ctrl:true},function(){copySelection();d3.event.preventDefault();});
            VARAI.keyboard.add(/* x */ 88,{ctrl:true},function(){copySelection();deleteSelection();d3.event.preventDefault();});
        }
        if (moving_set.length == 0) {
            VARAI.keyboard.remove(/* up   */ 38);
            VARAI.keyboard.remove(/* down */ 40);
            VARAI.keyboard.remove(/* left */ 37);
            VARAI.keyboard.remove(/* right*/ 39);
        } else {
            VARAI.keyboard.add(/* up   */ 38, function() { d3.event.shiftKey?moveSelection(  0,-20):moveSelection( 0,-1);d3.event.preventDefault();},endKeyboardMove);
            VARAI.keyboard.add(/* down */ 40, function() { d3.event.shiftKey?moveSelection(  0, 20):moveSelection( 0, 1);d3.event.preventDefault();},endKeyboardMove);
            VARAI.keyboard.add(/* left */ 37, function() { d3.event.shiftKey?moveSelection(-20,  0):moveSelection(-1, 0);d3.event.preventDefault();},endKeyboardMove);
            VARAI.keyboard.add(/* right*/ 39, function() { d3.event.shiftKey?moveSelection( 20,  0):moveSelection( 1, 0);d3.event.preventDefault();},endKeyboardMove);
        }
        if (moving_set.length == 1) {
            VARAI.sidebar.info.refresh(moving_set[0].n);
        } else {
            VARAI.sidebar.info.clear();
        }
    }
    ...
    function deleteSelection() {
        ...
        if (moving_set.length > 0) {
            for (var i in moving_set) {
                ...
                var rmlinks = VARAI.nodes.remove(node.id);
    ...
    function copySelection() {
        if (moving_set.length > 0) {
            ...
            for (var n in moving_set) {
                ...
                nns.push(VARAI.nodes.convertNode(node));
            }
            clipboard = JSON.stringify(nns);
            VARAI.notify(moving_set.length+" node"+(moving_set.length>1?"s":"")+" copied");
        }
    }
    ...
    function portMouseDown(d,portType,portIndex) {
        ...
        mousedown_node = d;
        selected_link = null;
        mouse_mode = VARAI.state.JOINING;
    ...
    function portMouseUp(d,portType,portIndex) {
        ...
        if (mouse_mode == VARAI.state.JOINING && mousedown_node) {
            ...
            VARAI.nodes.eachLink(function(d) {
                    existingLink = existingLink || (d.source === src && d.target === dst && d.sourcePort == src_port);
            });
            if (!existingLink) {
                var link = {source: src, sourcePort:src_port, target: dst};
                VARAI.nodes.addLink(link);
                VARAI.history.push({t:'add',links:[link],dirty:dirty});
                setDirty(true);
            }
            selected_link = null;
            redraw();
    ...
    function nodeMouseDown(d) {
        //var touch0 = d3.event;
        //var pos = [touch0.pageX,touch0.pageY];
        //VARAI.touch.radialMenu.show(d3.select(this),pos);
        if (mouse_mode == VARAI.state.IMPORT_DRAGGING) {
            VARAI.keyboard.remove(/* ESCAPE */ 27);
            updateSelection();
            setDirty(true);
        ...
        if (d.selected && d3.event.ctrlKey) {
            ...
            if (d3.event.shiftKey) {
                clearSelection();
                var cnodes = VARAI.nodes.getAllFlowNodes(mousedown_node);
                for (var i in cnodes) {
    ...
    function showTouchMenu(obj,pos) {
        ...
        options.push({name:"edit",disabled:(moving_set.length != 1),onselect:function() { VARAI.editor.edit(mdn);}});
        options.push({name:"select",onselect:function() {selectAll();}});
        options.push({name:"undo",disabled:(VARAI.history.depth() == 0),onselect:function() {VARAI.history.pop();}});
        
        VARAI.touch.radialMenu.show(obj,pos,options);
        resetMouseVars();
    }
    function redraw() {
        ...
        if (mouse_mode != VARAI.state.JOINING) {
            ...
            var node = vis.selectAll(".nodegroup").data(VARAI.nodes.nodes.filter(function(d) { return d.z == activeWorkspace }),function(d){return d.id});
            node.exit().remove();
                 ...
                 /*   var status = node.append("svg:g").attr("class","node_status_group").style("display","none");
                    ...
                    if (d._def.inputs > 0) {
                        ...
                        node.append("rect").attr("class","port port_input").attr("rx",3).attr("ry",3).attr("x",-5).attr("width",10).attr("height",10)
                            ...
                            .on("mouseover",function(d) { var port = d3.select(this); port.classed("port_hovered",(mouse_mode!=VARAI.state.JOINING || mousedown_port_type != 1 ));})
            ...
            node.each(function(d,i) {
                    if (d.dirty) {
                        ...
                        d._ports.enter().append("rect").attr("class","port port_output").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                            ...
                            .on("mouseover",function(d,i) { var port = d3.select(this); port.classed("port_hovered",(mouse_mode!=VARAI.state.JOINING || mousedown_port_type != 0 ));})
        ...
        var link = vis.selectAll(".link").data(VARAI.nodes.links.filter(function(d) { return d.source.z == activeWorkspace && d.target.z == activeWorkspace }),function(d) { return d.source.id+":"+d.sourcePort+":"+d.target.id;});
    ...
    VARAI.keyboard.add(/* z */ 90,{ctrl:true},function(){VARAI.history.pop();});
    VARAI.keyboard.add(/* a */ 65,{ctrl:true},function(){selectAll();d3.event.preventDefault();});
    VARAI.keyboard.add(/* = */ 187,{ctrl:true},function(){zoomIn();d3.event.preventDefault();});
    VARAI.keyboard.add(/* - */ 189,{ctrl:true},function(){zoomOut();d3.event.preventDefault();});
    VARAI.keyboard.add(/* 0 */ 48,{ctrl:true},function(){zoomZero();d3.event.preventDefault();});
    VARAI.keyboard.add(/* v */ 86,{ctrl:true},function(){importNodes(clipboard);d3.event.preventDefault();});
    VARAI.keyboard.add(/* e */ 69,{ctrl:true},function(){showExportNodesDialog();d3.event.preventDefault();});
    VARAI.keyboard.add(/* i */ 73,{ctrl:true},function(){showImportNodesDialog();d3.event.preventDefault();});
    ...
    function importNodes(newNodesStr,touchImport) {
        try {
            var result = VARAI.nodes.import(newNodesStr,true);
            if (result) {
                ...
                if (!touchImport) {
                    mouse_mode = VARAI.state.IMPORT_DRAGGING;
                }

                VARAI.keyboard.add(/* ESCAPE */ 27,function(){
                        VARAI.keyboard.remove(/* ESCAPE */ 27);
                        clearSelection();
                        VARAI.history.pop();
                        mouse_mode = 0;
                ...
                VARAI.history.push({t:'add',nodes:new_node_ids,links:new_links,dirty:VARAI.view.dirty()});
        ...
        buttons: [
            {
                ...
                click: function() {
                    var workspace = $(this).dialog('option','workspace');
                    VARAI.view.removeWorkspace(workspace);
                    var historyEvent = VARAI.nodes.removeWorkspace(workspace.id);
                    historyEvent.t = 'delete';
                    historyEvent.dirty = dirty;
                    historyEvent.workspaces = [workspace];
                    VARAI.history.push(historyEvent);
                    VARAI.view.dirty(true);
                    $( this ).dialog( "close" );
                }
            },
    ...
    return {
        ...
        status: function(s) {
            showStatus = s;
            VARAI.nodes.eachNode(function(n) { n.dirty = true;});
